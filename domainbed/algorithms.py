...
class DoPrompt_group_decompose(ERM):
    ...
    def taskprompt_decomposition(self):
        shared_prompt = self.shared_prompt_Tokens
        alpha = self.prompt_tokens_u
        gamma = self.prompt_tokens_v
        prompt_list = []
        for k in range(self.num_domains):
            alpha_k = torch.squeeze(alpha[k, :], dim=0)
            gamma_k = torch.squeeze(gamma[k, :], dim=0)
            prompt_k = shared_prompt * torch.ger(alpha_k, gamma_k)
    ...
    def minibatch_domain_prompt(self, minibatches, task_prompts):
        ...
        domain_tokens = task_prompts[domain_labels]
        return domain_tokens
